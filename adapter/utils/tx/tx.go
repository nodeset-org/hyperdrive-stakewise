...
func HandleTx(c *cli.Context, hd *swclient.HyperdriveClient, txInfo *eth.TransactionInfo, confirmMessage string, identifier string, submissionMessage string) (bool, error) {
...
func HandleTxWithIAgree(c *cli.Context, hd *swclient.HyperdriveClient, txInfo *eth.TransactionInfo, confirmMessage string, identifier string, submissionMessage string) (bool, error) {
...
func handleTxImpl(c *cli.Context, hd *swclient.HyperdriveClient, txInfo *eth.TransactionInfo, confirmMessage string, identifier string, submissionMessage string, useIAgree bool) (bool, error) {
	...
	// Assign max fees
	maxFee, maxPrioFee, err := utils.GetMaxFees(c, hd, txInfo.SimulationResult)
	if err != nil {
...
func HandleTxBatch(c *cli.Context, hd *swclient.HyperdriveClient, txInfos []*eth.TransactionInfo, confirmMessage string, identifierFunc func(int) string, submissionMessage string) (bool, error) {
	// Overwrite the gas limit if requested
	if c.IsSet(utils.ForceGasLimitFlag.Name) {
		manualLimit := c.Uint64(utils.ForceGasLimitFlag.Name)
	...
	maxFee, maxPrioFee, err := utils.GetMaxFees(c, hd, simResult)
	if err != nil {
		return false, fmt.Errorf("error getting fee information: %w", err)
	}
...
func waitForTransactions(hd *swclient.HyperdriveClient, hashes []common.Hash, identifierFunc func(int) string) error {
...
func updateCustomNonce(hd *swclient.HyperdriveClient) {
	if hd.Context.Nonce.Cmp(common.Big0) > 0 {
