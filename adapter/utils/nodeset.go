...
func CheckRegistrationStatus(c *cli.Context, hd *swclient.HyperdriveClient) (bool, error) {
	...
	if !Confirm("Would you like to register your node now?") {
...
func registerNodeImpl(c *cli.Context, hd *swclient.HyperdriveClient) error {
	// Get the email
	email := c.String(RegisterEmailFlag.Name)
	if email == "" {
		for {
			email = Prompt("Enter the email address you'd like to register with NodeSet:", "^.*$", "Invalid email address, try again")
