...
func NewSplitMain(address common.Address, ec eth.IExecutionClient, txMgr *eth.TransactionManager) (*SplitMain, error) {
	// Parse the ABI
	var err error
	splitMainOnce.Do(func() {
		var parsedAbi abi.ABI
		parsedAbi, err = abi.JSON(strings.NewReader(splitMainAbiString))
		if err == nil {
			splitMainAbi = parsedAbi
		}
	})
	if err != nil {
	...
	// Create the contract
	contract := &eth.Contract{
		ContractImpl: bind.NewBoundContract(address, splitMainAbi, ec, ec, ec),
		Address:      address,
		ABI:          &splitMainAbi,
	}
