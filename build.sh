...
# This script will build all of the artifacts involved in a new Stakewise daemon release.
...
build_daemon() {
    ...
    docker buildx build --rm --platform=linux/amd64,linux/arm64 -f docker/daemon-build.dockerfile --output build/$VERSION --target daemon . || fail "Error building Stakewise daemon binaries."
    echo "done!"
    ...
    # If uploading, make and push a manifest
    if [ "$UPLOAD" = true ]; then
        docker buildx build --rm --platform=linux/amd64,linux/arm64 --build-arg BINARIES_PATH=build/$VERSION -t nodeset/hyperdrive-stakewise:$VERSION -f docker/daemon.dockerfile --push . || fail "Error building Stakewise Docker image."
    elif [ "$LOCAL_UPLOAD" = true ]; then
        if [ -z "$LOCAL_DOCKER_REGISTRY" ]; then
            fail "LOCAL_DOCKER_REGISTRY must be set to upload to a local registry."
        fi
        docker buildx build --rm --platform=linux/amd64,linux/arm64 --build-arg BINARIES_PATH=build/$VERSION -t $LOCAL_DOCKER_REGISTRY/nodeset/hyperdrive-stakewise:$VERSION -f docker/daemon.dockerfile --push . || fail "Error building Stakewise Docker image."
    else
        docker buildx build --rm --load --build-arg BINARIES_PATH=build/$VERSION -t nodeset/hyperdrive-stakewise:$VERSION -f docker/daemon.dockerfile . || fail "Error building Stakewise Docker image."
    fi
    echo "done!"
}
...
tag_latest() {
    ...
    if [ "$UPLOAD" = true ]; then
        ...
        docker push nodeset/hyperdrive-stakewise:latest
        echo "done!"
    else
